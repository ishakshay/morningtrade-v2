...
from screeners.nse_sector_indices import sector_indices_bp, start_sector_polling
...
def export_eod_spreadsheet():
    ...
    hdr_fill_ce = PatternFill('solid', start_color='C0392B')
    hdr_fill_pe = PatternFill('solid', start_color='27AE60')
    hdr_fill_nt = PatternFill('solid', start_color='2C3E50')
    hdr_fill_vx = PatternFill('solid', start_color='8E44AD')
    ...
    for symbol in ['NIFTY', 'BANKNIFTY']:
        ...
        for r_idx, snap in enumerate(snaps, start=4):
            ...
            if not row_atm:
                continue
            try:
                col_offset = latest_strikes.index(int(row_atm))
            except ValueError:
                continue
...
app.register_blueprint(sector_indices_bp)   # ← NEW (must be after app = Flask())
start_sector_polling(60)                   # ← NEW (starts background thread)
...
_prev_pcr_3strike = {}
_prev_chain_cache = {}
_strike_iv_history = {'NIFTY': [], 'BANKNIFTY': []}
